...
import Data.Time(utctDayTime, getCurrentTime)
...
testCase :: String -> TestResult -> TestTree
testCase = Single

testProperty :: Testable a => String -> a -> TestTree
testProperty label a = Single label $ go quick (evaluate a) (mkStdGen 0) 0 0
  where
    go :: Config -> Gen Result -> StdGen -> Int -> Int -> TestResult
    go config gen rnd0 ntest nfail
      | ntest == maxTest config = Success
      | ntest == maxFail config = Success -- XXX
